...
c      PARAMETER (MAXCON=20)
c      PARAMETER (MAXDAT= 220 000 000, MAXMTR=1 000 000, MAXFRM=350)      !str                 SMONGO
c
c      PARAMETER (MAXCON=20)
c      PARAMETER (MAXDAT= 534 250 000, MAXMTR=  200 000, MAXFRM= 3770)    !danish98 + danish95   SMONGO

c
c      PARAMETER (MAXCON=20)
c      PARAMETER (MAXDAT= 530 000 000, MAXMTR=  400 000, MAXFRM= 2000)    !danish95 + danish96   SMONGO
c 
c      PARAMETER (MAXCON=20)
c      PARAMETER (MAXDAT=  534 250 000, MAXMTR= 500 000, MAXFRM= 1980)    !danish98            SMONGO
c      PARAMETER (MAXCON=20)
c      PARAMETER (MAXDAT=  400 000 000, MAXMTR=2 000 000, MAXFRM=50)    ! ACS   SMONGO

      PARAMETER (MAXCON=20)
      PARAMETER (MAXDAT=  10 000 000, MAXMTR= 500 000, MAXFRM=200)    
c
c (oYo)
C
...
C
      MAXSTR = MAXDAT/6
      MAXMAX = MAXDAT - 6*NTOT
c      write(6,*) 'memory check:',(MAXMAX-33*MAXSTR)/(NFRM+3),MAXMAX/(NFRM+7)
...
C
      ...
      PARAMETER (MAXFRM= 1980, MAXCON=20, MINY=-11 384, MAXY= 15 000)
c (oYo)
c
C
...
c         call sm_device ('x11 -bg black -fg yellow -focus')
         call sm_graphics
         call sm_expand (1.2)
         call sm_erase
         call sm_alpha
...
C
      TEXT = COMMAS(LIMIT, I)
      TEXT = '  '//TEXT(1:I)
      WRITE (TEXT(I+3:I+52),2) char(0)//'ZStarting  ', IFRM, FILE(IFRM)
...
C
         ...
         iii = 0
         do 2750 imastr=1,nmastr
            if (nobs(imastr) .lt. minuse) go to 2750
            if (mm(imastr) .gt. 90.) go to 2750
            if (iwhich(imastr,ifrm) .gt. 0) iii = iii+1
 2750    continue
         nfold = max0(1,iii/1500)
      end if
C
...
C        print 666,(DCON(I) / (1. + DABS(DCON(I)/CLAMP(I,IFRM))),i=1,iii)
         ...
         if (pause) then
            if ((text(1:1).eq.'e').or.(text(1:1).eq.'E')) then
               plot = .false.
            else if ((text(1:1).eq.'r').or.(text(1:1).eq.'R')) then
               pause = .false.
            end if
         end if
      end if
...
c ~~~~~~~~~~~~~~~~~~~~~~~~~
c
c	open (99, file='tempora.mch', status='unknown')
         DO I=1,NFRM
            IF (MODE .EQ. 4) THEN
               CON(6,I)=FLIP(I)*CON(3,I)
               CON(5,I)=-FLIP(I)*CON(4,I)
...
C
            IF (MODE .LE. 6) THEN
c               WRITE (99,1109) FILE(I), (con(J,i), J=1,6), 
c     .              DMAG(I), SQRT(SMAG(I))
c               WRITE (99,1109) FILE(I), (OUT(J), J=1,6), 
c     .              DMAG(I), SQRT(SMAG(I))
            ELSE
               DO J=7,MODE
                  OUT(J) = DRNDFF(CON(J,I), 12, 9)
               END DO
               WRITE (99,1109) FILE(I), (con(J,i), J=1,6), 
...
c  111       FORMAT (I7, 2A9, 12F9.4:/ (24X, 12F9.4))
  ...
  111       FORMAT (I7, f9.3, 2x, f9.3, 12F9.4:/ (24X, 12F9.4))
         END DO
         CALL CLFILE (1)
      END IF
C
 ...
 1020    CONTINUE
  ...
  101       FORMAT (26X, F8.4)
         ...
         ELSE
            ...
            READ (TEXT,*) ID, (WORK(ISTAR,J), J=1,4),
C           READ (TEXT,*,ERR=1020) ID, (WORK(ISTAR,J), J=1,4),
