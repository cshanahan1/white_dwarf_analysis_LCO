...
C
      CHARACTER FILE*30, OFILE*30, SWITCH*30, EXTEND*30
...
C
      ...
      DATA TWOPI/6.283184/
      DATA XYLIM /8000./, SCTOL/0.01/, PSTOL/0.3/
      CALL FABORT
...
C
 ...
 1019 CALL GETNAM ('Master input file:', FILE)
      ...
      DO K=30,2,-1
         IF (FILE(K:K) .EQ. '%') THEN
            READ (FILE(K+1:30),*) CHIMAX
c           L = K
            FILE(K:30) = ' '
         ELSE IF (FILE(K:K) .EQ. '<') THEN
            READ (FILE(K+1:30),*) MAGMAX
            L = K
            FILE(K:30) = ' '
         ELSE IF (FILE(K:K) .EQ. '[') THEN
            READ (FILE(K+1:30),*) SHPMAX
            L = K
            FILE(K:30) = ' '
         ELSE IF (FILE(K:K) .EQ. '|') THEN
            READ (FILE(K+1:30),*) SIGMAX
c           L = K
            FILE(K:30) = ' '
         ELSE IF (FILE(K:K) .EQ. '/') THEN
            READ (FILE(K+1:30),*) SIGMASQ
            SIGMASQ = SIGMASQ**2
c           L = K
            FILE(K:30) = ' '
         ELSE IF (FILE(K:K) .EQ. '*') THEN
...
C
     ...
     .     MAXSTR+10, X(1,1), Y(1,1), MAG(1,1), N1, WT, TEMP, IA)
      N1 = MIN(N1, MAXSTR)
 1020 CALL CLFILE (2)
...
C
      DO K=29,2,-1
         IF (FILE(K:K) .EQ. '/') THEN
            IF (FILE(K+1:30) .EQ. ' ') THEN
               SCALE = 1.
            ELSE
               READ (FILE(K+1:30),*) SCALE
            END IF
            SETSCA = .TRUE.
            SETROT = .FALSE.
            FILE(K:30) = ' '
...
C
 9000 CONTINUE
      IF (JF .LE. 1) THEN
         WRITE (1,101) FILE, H(1), H(2), H(3), H(4), -H(4), H(3), FRAD,
     .     0.
      ELSE
         WRITE (1,101) FILE, H(1), H(2), H(3), H(4), H(4), -H(3), FRAD,
     .     0.
...
C
     ...
     .     SIGMAX, MAX, X, Y, M, N, G, HOLD, INDEX)
      CHARACTER LINE*132
...
C
 ...
 1015    CALL RDCHAR (2, LINE, NC, ISTAT)
         ...
         LINE = LINE(1:NC)//' 0 0 0 0'
         READ (LINE,*,ERR=1015) II, XX, YY, MM, 
     .        SIG, DUM, DUM, CHI, SHP
         IF ((II .LE. 0) .OR. (CHI .GT. CHIMAX) .OR. 
     .        (SIG .GT. SIGMAX) .OR. (MM .GT. MAGMAX) .OR.
     .        (ABS(SHP) .GT. SHPMAX)) GO TO 1010
C
C-----------------------------------------------------------------------
C
         IF (LIMIT) THEN
