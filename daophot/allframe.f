...
C
      ...
      PARAMETER (MAXSTR= 150 000, MAXLST=  100 000) ! Efosc
      PARAMETER (MAXCOL= 4096, MAXROW= 4096, RAT=1.4)

c      PARAMETER (MAXSTR= 500 000, MAXLST= 1 000 000)   ! Simulazioni banda K
c      PARAMETER (MAXCOL=6100, MAXROW=6100, RAT=1.4)

c      PARAMETER (MAXSTR= 200 000, MAXLST=  500 000)   ! Simulazioni banda J
c      PARAMETER (MAXCOL=10000, MAXROW=10000, RAT=1.4)

c      PARAMETER (MAXSTR= 500 000, MAXLST= 20 000 000)   ! Simulazioni banda K + J
c      PARAMETER (MAXCOL=10100, MAXROW=10100, RAT=1.4)

c      PARAMETER (MAXSTR= 500 000, MAXLST= 5 000 000)   ! VIMOS
c      PARAMETER (MAXCOL=2500, MAXROW=2500, RAT=1.4)

c      PARAMETER (MAXSTR= 150 000, MAXLST= 1 000 000)   ! DECAM
c      PARAMETER (MAXCOL=2160, MAXROW=4146, RAT=1.4)
c

c      PARAMETER (MAXSTR= 730000, MAXLST = 150 000 000)     ! wfi tutto 


c      PARAMETER (MAXSTR= 720 000, MAXLST = 200 000 000)     ! wfi deep 
c      PARAMETER (MAXCOL= 2048, MAXROW= 4098, RAT= 2.30)

c      PARAMETER (MAXSTR= 50 000, MAXLST = 5 000 000)     ! Megacam
c      PARAMETER (MAXCOL= 2112, MAXROW= 4644, RAT= 1.4)

c      PARAMETER (MAXSTR= 950 000, MAXLST = 300 000 000)     ! wfi deep +Vlt +vimos
c      PARAMETER (MAXCOL= 2048, MAXROW= 4098, RAT= 1.4)
c
c      PARAMETER (MAXSTR= 350 000, MAXLST = 45 000 000)     ! FORS_HR 
c      PARAMETER (MAXCOL= 2100, MAXROW= 2100, RAT= 1)

c      PARAMETER (MAXSTR= 450 000, MAXLST = 30 000 000)     ! FORS_HR o VIMOS 
c      PARAMETER (MAXCOL= 2102, MAXROW= 2500, RAT= 1.4)

c      PARAMETER (MAXSTR= 560 000, MAXLST = 30 000 000)     ! Mad!!!!!!!!!!!
c      PARAMETER (MAXCOL= 4250, MAXROW= 4250, RAT= 1.4)

c      PARAMETER (MAXSTR= 15 000, MAXLST =  500 000)          ! Mad ALL !!!!!!!
c      PARAMETER (MAXCOL= 4250, MAXROW= 4250, RAT= 1.4)

c      PARAMETER (MAXSTR= 30 000, MAXLST =  50 000)          ! Mad NGC3201 !!!!!!!
c      PARAMETER (MAXCOL= 2100, MAXROW= 2100, RAT= 1.4)

c      PARAMETER (MAXSTR= 160000, MAXLST = 100 000 000)  !ngc3201 
c      PARAMETER (MAXCOL= 2048, MAXROW= 4098, RAT= 2.00)
c
c      PARAMETER (MAXSTR= 100 000, MAXLST = 90 000 000)             !danish 99 
c      PARAMETER (MAXCOL= 2102, MAXROW= 2102, RAT= 1.4)

c      PARAMETER (MAXSTR= 82 000, MAXLST = 163 000 000)             !danish 98 
c      PARAMETER (MAXCOL= 2102, MAXROW= 2102, RAT= 1.4)

c      PARAMETER (MAXSTR= 100 000, MAXLST = 50 000 000)             !danish 95 
c      PARAMETER (MAXCOL= 1050, MAXROW= 1050, RAT= 1.4)

c      PARAMETER (MAXSTR= 130 000, MAXLST = 230 000 000)             !danish 95+99 
c      PARAMETER (MAXCOL= 2102, MAXROW= 2102, RAT= 1.4)
...
C      43*MAXPIC/3.  If all stars are in all images, then 
...
C Functions
C
      CHARACTER EXPAND*132, EXTEND*30, SWITCH*30, COMMAS*30, CASE*30,
     .     SQUEEZ*30, RNDOFF*9, DRNDFF*11
      REAL AMIN1, SQRT, USEPSF, PCTILE
      INTEGER RDPSF, LENGTH, IPSTYP
C
C Arrays
C
      CHARACTER OUTPUT(MAXPIC/3+1)*30, LBL(NOPT)*26, STRNG(10)*10
...
C
      CHARACTER LINE*300
      CHARACTER OPTFIL*30, INPFIL*30, BACKUP*30, NEWFIL*30, TFRFIL*30
...
C     INTEGER MADRID                              ! MIDAS
      LOGICAL CENTER, CLIP, CONVRG, DOSKY, START
C
...
C
     ...
     .     (MOD(ISTAR,1000) .EQ. 0)) THEN
         LINE = ' '
         WRITE (LINE,84) ISTAR
   84    FORMAT (I7, ' stars checked for crowding.')
         CALL OVRWRT (LINE(1:70), 2)
...
C
  ...
  664       FORMAT (I7, 2F9.3, '  blended with ', I7, 4F9.3)
C
...
C
   ...
   91    FORMAT (I7, ' stars remain, ')
      ELSE IF (NSTAR .EQ. 1) THEN
         WRITE (LINE(9:30),92) NSTAR
   92    FORMAT (I7, ' star remains, ')
      ELSE
         WRITE (LINE(9:30),89)
...
C
   ...
   93    FORMAT (I7, ' have converged, ')
      ELSE IF (NCNVRG .EQ. 1) THEN
         WRITE (LINE(31:54),94) NCNVRG
   94    FORMAT (I7, '  has converged, ')
      ELSE
         WRITE (LINE(31:54),95)
   95    FORMAT ('    none has converged,')
      END IF
C
      IF (NDISAP .GT. 1) THEN
         WRITE (LINE(55:79),96) NDISAP
   96    FORMAT (I7, ' have disappeared.')
      ELSE IF (NDISAP .EQ. 1) THEN
         WRITE (LINE(55:79),97) NDISAP
   97    FORMAT (I7, '  has disappeared.')
      ELSE
         WRITE (LINE(55:79),98)
   98    FORMAT ('   none has disappeared.')
      END IF
C
...
C
      DO 5000 IMNO=1,NPIC
C
      IF ((NSTAR .GT. 100) .AND. (WATCH .GT. -1.5)) THEN
         LINE = ' '
...
C
 ...
 2098    IF ((WATCH .GT. 0.5) .AND. (MOD(ISTAR,INTRVL) .EQ. 0)) THEN
   ...
   81       FORMAT (I7, ' stars subtracted from image ',
     .           I3, ':  ', A30)
            CALL OVRWRT (LINE(1:70), 2)
         END IF
 2099 CONTINUE
...
C
 2198       IF ((WATCH .GT. 0.5) .AND. (MOD(ISTAR,INTRVL) .EQ. 0)) THEN
   ...
   82          FORMAT (I7, ' sky values redetermined for ',
     .              'image ', I3, ':  ', A30)
               CALL OVRWRT (LINE(1:70), 2)
            END IF
 2199    CONTINUE
...
C
            ...
            IF (XMAX .GT. 0.05*AMAG(KSTAR)) THEN
               DONE(ISTAR) = .FALSE.
            ELSE IF (XMAX**2 .GT. 0.004*MAGERR(KSTAR)) THEN
               DONE(ISTAR) = .FALSE.
            END IF
         END IF
C
...
C
 2299    IF ((WATCH .GT. 0.5) .AND. (MOD(ISTAR,INTRVL) .EQ. 0)) THEN
   ...
   83       FORMAT (I7, ' stars corrected for image ',
     .           I3, ':  ', A30)
            CALL OVRWRT (LINE(1:70), 2)
         END IF
 2300 CONTINUE
...
C
 ...
 4500 CONTINUE
      MX = NTFM
      IF (L .LE. 2) MX = MIN(1, MX)
      IF (L .LE. 6) MX = MIN(3, MX)
      IF (L .LE. 10) MX = MIN(6, MX)
CC
...
C      END DO
      ...
      DO I=1,MX
         K = 2*I
         J = K-1
         FFEOC(J,IMNO) = FFEOC(J,IMNO) + ZX(I) / (1.D0+DABS(ZX(I)))
         FFEOC(K,IMNO) = FFEOC(K,IMNO) + ZY(I) / (1.D0+DABS(ZY(I)))
      END DO
...
C
     ...
     .     (MOD(ISTAR,1000) .EQ. 0)) THEN
         LINE = ' '
         WRITE (LINE,85) ISTAR
   85    FORMAT (I7, ' stars checked for validity.')
         CALL OVRWRT (LINE(1:70), 2)
      ELSE IF ((WATCH .GT. -1.5) .AND. 
     .     (MOD(ISTAR,10000) .EQ. 0)) THEN
         LINE = ' '
...
C
      ...
      IF ((WATCH .GT. -1.5) .AND. (NSTAR .GT. 50)) THEN
         LINE = ' '
         WRITE (LINE,87)
...
*         OPEN (1, FILE=EXPAND(NEWFIL), STATUS='OLD', position='APPEND') ! pgf90 
      END IF
C
      ...
      IF (DPOS .GE. 0.3995) THEN
c      IF (DPOS .GE. 9.99995) THEN            !variazione da utilizzare per l'allframe finale, cosi' si prendono tutte le 
         MAGERR(KSTAR) = -10.                 ! misure , anche quelle piu' discrepanti
         GO TO 6004
      END IF
C
