...
c      PARAMETER (MXINC=8192, MXOUTC=12288, MXOUTR=12288)
      PARAMETER (MXINC=15000, MXOUTC=15288, MXOUTR=15288)
c      PARAMETER (MXINC=32768, MXOUTC=49152, MXOUTR=49152)  ! ACS mosaic 
C
      CHARACTER FILE(MAXPIC)*30, NAME*30, SUFF*30, SWITCH*30, 
     .     EXTEND*30, SQUEEZ*30, TEMP*30
      CHARACTER LINE*300, COMMAS*19, OFFSCA*19, CASE*5, ANSWER*1
     ...
     .     LOBAD(MAXPIC)
      ...
      LOGICAL ALIGN(MAXPIC), ROTATE, USEPDS
C
...
C
  ...
  755 CALL RDCHAR (1, LINE, N, IER)
     ...
     .           (COEFF(J,NFRAME), J=1,6), DMAG(NFRAME)
      ...
      END IF
C
C Ignore all .stc, .mag, .nmg, .fet and .dss files.
C
      do i=1,27
...
C
C Fluxes must be scaled by sampling interval squared to from DN per
C arcsecond to DN per pixel
C
...
C
    ...
    6    FORMAT (1X, I5, 2(I9, '   ', I6), I8, ' x', I6, '    <<')
         IF ((IDX .GT. 0) .AND. (IDX*IDY .GT. MINSKY)) J = I
      END DO
C
      IF (J .LE. 0) THEN
         CALL STUPID (' Insufficient overlap to compare sky levels.')
         CALL OOPS
      END IF
C
      CALL TBLANK
 ...
 1901 CALL GETDAT ('X limits of output image:', DATUM, 2)
      ...
      IEXPND = NINT(DATUM(1))
      IEXPND = MIN0( IEXPND, MXOUTC/(IXMAX-IXMIN+1),
     .     MXOUTR/(IYMAX-IYMIN+1) )
...
C
      DO IMNO=1,NFRAME
         ...
         WRITE (6,10) NINT(SKY(IMNO)), HOLD(IMNO),
     .        WEIGHT(IMNO),
...
C
 ...
 1990 I = LENGTH(NAME)
      IF (NFRAME .GT. 1) THEN
         ...
         WRITE (LINE,199) NAME(1:I), TEMP(1:J)
  199    FORMAT (A: ' - Montage of ', A, ' images')
      ELSE
         WRITE (LINE,199) NAME(1:I)
      END IF
      J = LENGTH(LINE)
...
C
      DO IMNO=1,NFRAME
         IF (ABS(FFEOC(4,IMNO)*FFEOC(5,IMNO)) .GT. 
     .        ABS(FFEOC(3,IMNO)*FFEOC(6,IMNO))) THEN
            ALIGN(I) = .FALSE.
         ELSE
            ALIGN(I) = .TRUE.
         END IF
...
C
            IF (ALIGN(IMNO)) THEN
               CALL RDROW (IMNO, J, ROW(1,IMNO), IER)
               LROW(IMNO) = J
            ELSE
...
C
 2498       DD = ROW(I,IMNO)
     ...
     .           (DD .LE. HIBAD(IMNO))) THEN
            ...
            END IF
 2500     CONTINUE
C2501     IF ((M .LT. NMIN) .OR. (M .GT. NMAX)) THEN
 2501     CONTINUE
...
C
 ...
 4000 CONTINUE
      ...
      DO IROW=K,KK,ISTEP
         Y = REAL(IROW)
         CALL RDROW (IMNO, IROW, WORK(I+1), IER)
         IF (IER .NE. 0) THEN
