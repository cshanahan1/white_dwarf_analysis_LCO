...
C
      ...
      PARAMETER  (MAXBOX=69, MAXSTR=450000, MAXN=400) 
C
C Parameters
C
...
C
      ...
      CHARACTER COOFIL*30, MAGFIL*30, PSFFIL*30, FITFIL*30, 
     .     GRPFIL*30, SWITCH*30
...
C
 ...
 3450 CONTINUE
   ...
   66 FORMAT (/' Profile errors:'/)
	open (65, FILE="lista.dat", STATUS='UNKNOWN')                       !(oYo)
  	   CALL WRHEAD (65, 1, NCOL, NROW, 7, LOBAD, HIBAD, THRESH, AP1,    !(oYo) 
     .     PHPADU, READNS, RADIUS)                                          !(oYo) 
      DO I=1,K
         LINE = ' '
         DO ISTAR=I,NSTAR,K
            J = 17*(ISTAR-I)/K + 1
...
C
   96          FORMAT (I7, 2x, 4(F8.3, 2x), 1X, A1, 1X, F8.3)               !(oYo)
             if (HJNK(ISTAR).lt.0.07.and.HJNK(ISTAR).gt.0)                  !(oYo)
     .	        WRITE (65,96) ID(ISTAR),                                    !(oYo)
     .	        xcen(istar),ycen(istar),apmag(istar), sky(istar),           !(oYo)
     .	        FLAG(ISTAR),HJNK(ISTAR)                                     !(oYo)
C
         END IF
         END DO
         WRITE (6,6) LINE(1:85)
...
c      CALL TBLANK
c      WRITE (6,*) 'File with PSF stars and neighbors = ', MAGFIL
c 2920 CALL OUTFIL (3, MAGFIL, ISTAT)
c      IF (ISTAT .NE. 0) THEN
c         CALL STUPID ('Error opening output file '//MAGFIL)
c         MAGFIL = 'GIVE UP'
...
c      END IF
cC
cC First, write out the PSF stars.
cC
c      IF (RADIUS .GT. 0.001) THEN
c         I = 7
c      ELSE
c         I = 6
c      END IF
cC
c      CALL WRHEAD (3, 3, NCOL, NROW, I, LOBAD, HIBAD, THRESH, AP1, 
c     .     PHPADU, READNS, RADIUS)
c      DO I=1,NSTAR
c         IF ((SKY(I) .GT. -999.949) .AND. (SKY(I) .LT. 9999.949)) THEN
c            WRITE (3,280) ID(I), XCEN(I), YCEN(I), APMAG(I), SKY(I),
c     .           FLAG(I)
c 280        FORMAT (I7, 4F9.3, 3X, A1)
c         ELSE IF ((SKY(I) .GT. -9999.949) .AND. 
c     .                                    (SKY(I) .LT. 99999.94)) THEN
c            WRITE (3,281) ID(I), XCEN(I), YCEN(I), APMAG(I), SKY(I),
c     .           FLAG(I)
c 281        FORMAT (I7, 3F9.3, F9.2, 3X, A1)
c         ELSE
c            WRITE (3,282) ID(I), XCEN(I), YCEN(I), APMAG(I), SKY(I),
c     .           FLAG(I)
c 282        FORMAT (I7, 3F9.3, F9.1, 3X, A1)
c         END IF
c      END DO
c      IF (NSTAR .GE. NTOT) GO TO 9000
cC
cC Now look for all stars within a radius equal to 
cC 1.5*PSFRAD+2.*FITRAD+1 of any of the PSF stars.
cC
c      RSQ = (1.5*PSFRAD+2.*FITRAD+1.)**2
c      L = NSTAR+1
cC
cC L will point at the first irrelevant star.
cC
c      DO I=1,NSTAR
c         J = L
c 5020    IF ((XCEN(J)-XCEN(I))**2 + (YCEN(J)-YCEN(I))**2 .LE. RSQ) THEN
c            CALL SWAPP (ID, XCEN, YCEN, APMAG, SKY, J, L)
c            WRITE (3,280) ID(L), XCEN(L), YCEN(L), APMAG(L), SKY(L)
c            L = L+1
c         END IF
c         IF (J .LT. NTOT) THEN
c            J = J+1
...
c      END DO
cC
cC Now all stars in the stack between positions NSTAR+1 and L-1,
cC inclusive, lie within the specified radius of the PSF stars and have
cC been written out.  Finally, look for any stars within a radius 
cC equal to 2*FITRAD+1 of any of THESE stars, and of each other.
cC
c      IF (L .GT. NTOT) GO TO 9000
c      RSQ = (2.*FITRAD+1)**2
c      I = NSTAR+1
c 5110 J = L
c 5120 IF ((XCEN(J)-XCEN(I))**2 + (YCEN(J)-YCEN(I))**2 .LE. RSQ) THEN
c         CALL SWAPP (ID, XCEN, YCEN, APMAG, SKY, J, L)
c         WRITE (3,280) ID(L), XCEN(L), YCEN(L), APMAG(L), SKY(L)
c         L = L+1
c      END IF
...
cC
c      I = I+1
c      IF (I .LT. L) GO TO 5110
c 9000 CALL CLFILE (3)
      RETURN
      END
C
C#######################################################################
...
C
      ...
      PARAMETER  (MAXPAR=6, MAXBOX=69, MAXN=400)
      CHARACTER LINE*80
      REAL PIC(NCOL,*)
