...
C
      PARAMETER (MAXAP=12, MAXSTR=250, MAXFIL=2000, MTERM=5, NOPT=14)
...
C
      CALL OVRWRT (' ', 4)
      CALL STUPID ('Converged.')
C
      WRITE (3,304) P
  304 FORMAT (1X, '--> A, B, C, D, E =', 5F13.6)
      WRITE (3,305) (SUMN*DSQRT(U(J,J)), J=1,5)
  305 FORMAT (1X, '-->              +-', 5F13.6)
      WRITE (3,306)
      WRITE (3,306) '     Radii', (R(J), J=1,NDIFF)
  306 FORMAT (1X, '-->', 9X, A10, 12F9.4)
      WRITE (3,306)
      WRITE (6,6663) P
 6663 FORMAT (1X, 5F13.6)
      WRITE (6,6663) (SUMN*DSQRT(U(J,J)), J=1,5)
      CALL TBLANK
      SUMD = 0.
      SUMR = 0.
      SUMRSQ = 0.
...
C
  ...
  300    FORMAT (//1X, '-->', I5.4, 1X, A, '   <sky> =', F9.1,
     .        5X, 'Ro =', F14.7/)
         WRITE (3,301) K, '   Model', (THEO(J), J=2,NDIFF)
  301    FORMAT (1X, '-->', I5.4, 10X, A8, 12F9.4)
         WRITE (3,301) K
  ...
  303    FORMAT (1X, '-->', I5.4, 4X, A10, 12F9.4)
...
c    .sqrt(wcum(j+1)),w(j),sigsq,sigmin
  ...
  222       FORMAT (I7, 2A9, 2F9.4, A9, F9.3, F9.4, I9)
     ...
     .           (OBS(J), J=1,NAP(ISTAR))
  302       FORMAT (I6, 2F8.2, ':', F8.3,  11F9.3)
            WRITE (3,333) (WOBS(J), J=1,NAP(ISTAR))
  333       FORMAT (23X, 12F9.4)
         END DO
         WRITE (3,301)
         CALL CLFILE (2)
