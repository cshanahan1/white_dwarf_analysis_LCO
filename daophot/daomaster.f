...
c      PARAMETER (MAXDAT=150 000 000, MAXMTR=1 000 000, MAXFRM=555)
c      PARAMETER (MAXDAT=  300 000 000, MAXMTR=1 000 000, MAXFRM=650) !wfi_new
c      PARAMETER (MAXWHI=  500 000 000)                               !wfi_new

c       PARAMETER (MAXDAT=  500 000 000, MAXMTR = 1 500 000, MAXFRM= 750) !wfi_new
c       PARAMETER (MAXWHI=  500 000 000)                               !wfi_new

       PARAMETER (MAXDAT=  530 000 000, MAXMTR = 1 500 000, MAXFRM= 4935) !wfi_new
       PARAMETER (MAXWHI=  500 000 000)                               !wfi_new

c       PARAMETER (MAXDAT=  500 000 000, MAXMTR = 1 500 000, MAXFRM= 380)   !wfi + vlt
c       PARAMETER (MAXWHI=  500 000 000)                                    !wfi + vlt

c       PARAMETER (MAXDAT=  530 000 000, MAXMTR = 1 500 000, MAXFRM= 2025) ! danish98
c       PARAMETER (MAXWHI=  530 000 000)                                  ! danish98
...
C
      DATA  DUP/.TRUE./, PWT1/250./, STEP/2.5/
C
C=======================================================================
C
      CALL FABORT
C     CALL STUPID ('     MONGO version')
      DO I=1,MAXFRM
         DO J=1,MAXCON
            CON(J,I) = 0.D0
         END DO
...
C
      ...
      PARAMETER (MAXFRM= 4935, MAXCON=20, MINY=-11 384, MAXY=15 000)
C
...
d        wfc(i) = .false.
d        do j=2,29
d           if (file(i)(j:j) .eq. ':') then
d              if ((file(i)(j-1:j-1) .eq. 'p') .and.
d    .             (file(i)(j+1:j+1) .eq. 'u')) pc(i) = .true.
d              if ((file(i)(j-1:j-1) .eq. 'w') .and.
d    .             (file(i)(j+1:j+1) .eq. 'u')) wfc(i) = .true.
d              go to 82
d           end if
d        end do
   82 CONTINUE
...
C
            ...
            END IF
         ELSE
            IF (IWHCH2(IMASTR,IFRM-MIDFRM) .GT. 0) THEN
C 
C ... but another star had already been matched with this star in the
C master list.  We know that this new star is a better match, so attempt
C to re-match the previous star.
C
               IBUMP=IWHCH2(IMASTR,IFRM-MIDFRM)
               IWHCH2(IMASTR,IFRM-MIDFRM)=III
               CALL TRFM (CON(1,IFRM), RCOL(IFRM), RROW(IFRM), 
     .              FLIP(IFRM), MODE, X(IBUMP), Y(IBUMP), XX, YY)
               III=IBUMP
               GO TO 2155
...
c     call ovrwrt ('Loop 2               ', 2)
     ...
     .           NOBS(IMASTR)=NOBS(IMASTR)+1
      ...
      DO IFRM=MIDFRM+1,NFRM
         DO IMASTR=1,NMASTR
            IF (IWHCH2(IMASTR,IFRM-MIDFRM) .GT. 0) 
     .           NOBS(IMASTR)=NOBS(IMASTR)+1
         END DO
      END DO
C
...
C
      ...
      SAD=-1.0
d     if (plot) then
d        if (ifrm.eq.ifirst) call stupid (' ')
d        a = 1.e20
d        b = -1.e20
d        do imastr=1,nmastr
d           if (ifrm .le. midfrm) then
d              l = iwhich(imastr,ifrm)
d           else
d              l = iwhch2(imastr,ifrm-midfrm)
d           end if
d           if (l .gt. 0) then
...
d        end do
d        iii = 0
d        do 750 imastr=1,nmastr
d           if (nobs(imastr) .lt. minuse) go to 750
d           if (mm(imastr) .gt. 90.) go to 750
d           if (ifrm .le. midfrm) then
d              if (iwhich(imastr,ifrm) .gt. 0) iii = iii+1
d           else
d              if (iwhch2(imastr,ifrm-midfrm) .gt. 0) iii = iii+1
d           end if
d 750    continue
d        nfold = max0(1,iii/1000)
d     end if
C
...
d    .        '  ', file(ifrm)(1:length(file(ifrm)))
d 666    format (12A / 24X, 11A)
d        if (pause) then
...
d              plot = .false.
d           else if ((text(1:1).eq.'g').or.(text(1:1).eq.'G')) then
d              pause = .false.
d           end if
d        end if
d     end if
...
C
  ...
  603 FORMAT (2X, A, ' stars within radius', A6)
      ...
      CALL TBLANK
*
*--------- addendum for temporary file including transformations 
*
	open (99, file='temporary.mch', status='unknown')
         DO I=1,NFRM
            IF (MODE .EQ. 4) THEN
               CON(6,I)=FLIP(I)*CON(3,I)
               CON(5,I)=-FLIP(I)*CON(4,I)
...
C
            IF (MODE .LE. 6) THEN
               WRITE (99,1109) FILE(I), (OUT(J), J=1,6), 
     .              DMAG(I), SQRT(SMAG(I))
            ELSE
               DO J=7,MODE
...
C
 3015       POSIT1 = RNDOFF(XM(IMASTR), 9, 3)
     ...
     .           CH, SHP, STRNG, RR
c  110       FORMAT (I7, 2A9, F9.3, 2F9.4, I8, '.', 3A9, F9.3)
  110       FORMAT (I7, 2(1x,A9), F9.3, 2F9.4, I8, '.', 3(1x,A9), F9.3)
         END DO
         CALL CLFILE (1)
      END IF
C
