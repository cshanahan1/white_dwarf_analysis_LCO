...
c      PARAMETER (MAXDAT=2 000 000 000, MAXMTR=1 400 000, MAXFRM=1206)
c      PARAMETER (MAXDAT=1 000 000 000, MAXMTR=1 300 000, MAXFRM=800)

      PARAMETER (MAXDAT= 500 000 000, MAXMTR= 1 500 000, MAXFRM= 400)       !wfi
 
c      PARAMETER (MAXDAT= 700 000 000, MAXMTR= 2 000 000, MAXFRM= 300)       !wfi_new

c      PARAMETER (MAXDAT= 200 000 000, MAXMTR= 300 000, MAXFRM= 6090)  ! ULTRACAM

c      PARAMETER (MAXDAT= 50 000 000, MAXMTR=  800 000, MAXFRM= 400)   !danish98
c      PARAMETER (MAXDAT= 1 500 000 000, MAXMTR=  800 000, MAXFRM= 1500)   !danish95

c      PARAMETER (MAXDAT= 500 000 000, MAXMTR= 1 500 000, MAXFRM= 380)   
 
c      PARAMETER (MAXDAT= 500 000 000, MAXMTR= 2 000 000, MAXFRM= 4935)    !danish tutto
      
c (oYo)
c
C
      CHARACTER FILE(MAXFRM)*30, EXTEND*30, SWITCH*30
      CHARACTER COMMAS*20
      CHARACTER CASE*3
      ...
      INTEGER IWORK(MAXDAT)
      BYTE BWORK(4*MAXDAT)
C
C      CHARACTER TEXT*300
      CHARACTER TEXT*330
      CHARACTER TEXT2*20
      CHARACTER LFILE*30
      REAL DUM, CHIMAX, SIGMAX, MAGMAX, SHPMAX, RX, RY, PWT1, STEP
      INTEGER I, J, N, NN, IFRM, MAXSTR, MAX, NFRM, NTOT, ISTAT
      LOGICAL TFR, DUP
C
C The big scratch array ?WORK, must be capable of holding data of type
C REAL*4, INTEGER*4, or BYTE*1.
C
      EQUIVALENCE (RWORK, IWORK), (RWORK, BWORK)
C
      DATA  DUP/.TRUE./, PWT1/500./, STEP/2.5/
C
C=======================================================================
C
      CALL FABORT
D     CALL STUPID ('     MONGO version')
      DO I=1,MAXFRM
         DO J=1,MAXCON
            CON(J,I) = 0.D0
         END DO
...
C
  ...
  890 CALL GETNAM ('File with list of input files:', LFILE)
     ...
     .        (LFILE(I:I).EQ.'\\')) THEN
            ...
            ELSE IF (LFILE(I:I) .EQ. '-') THEN
               IF (LFILE(I+1:I+1) .EQ. 'x') THEN
                  RX = -1.
                  IF (LFILE(I+2:I+2) .EQ. 'y') RY = -1.
               ELSE IF (LFILE(I+1:I+1) .EQ. 'y') THEN
                  RY = -1.
               END IF
            END IF
            LFILE = LFILE(1:I-1)
...
C
      ...
      IF (ISTAT .NE. 0) THEN
         CALL STUPID ('Error opening '//FILE(IFRM))
         CALL CLFILE (1)
         CALL OOPS
      END IF
      CALL CLFILE (2)
C
      IF (IFRM .GT. 1) THEN
         DO I=1,IFRM-1
            IF (SWITCH(FILE(IFRM), ' ') .EQ. 
     .          SWITCH(FILE(I), ' ')) THEN
               IF (DUP) CALL STUPID ('  Duplicate input files?')
               DUP = .FALSE.
               WRITE (6,*) FILE(I), FILE(IFRM)
               CALL TBLANK
            END IF
         END DO
      END IF
...
C
      MAXSTR = MAXDAT/6
      CALL RDFRMS (FILE, TFR, CHIMAX, SHPMAX, MAGMAX, SIGMAX, NFRM, 
     .     MAXMTR, RWORK, MAXSTR, IDMAST, LASTAR, RCOL, RROW)
C
...
C after we are completely finished with MM, ..., they may share
...
C
      MAX = MAXDAT - 6*NTOT
d     i = (MAX-33*MAXSTR)/(NFRM+3)
d     j = MAX/(NFRM+7)
d     text = commas(i, n)
d     text2 = commas(j, nn)
d     write (6,*) 'LINE limit ', text(1:n),
d    .     '  INDEX limit ', text2(1:nn)
d     call tblank
      MAX = MIN0 ( MAXMTR,
     .            (MAX-33*MAXSTR)/(NFRM+3),
     .             MAX/(NFRM+7) )
C
...
C
      N = 6*NTOT                               ! End of input data
      NN = N + MAX*NFRM                        ! End of IWHICH storage
C
...
C XM, YM, MM,
C
     .     RWORK(NN+1), RWORK(NN+MAX+1), RWORK(NN+2*MAX+1),
C
C NLINE, LINE,
C
     .     IWORK(NN+3*MAX+1), BWORK(4*(NN+3*MAX+MAXSTR)+1),
C
C SM, RMIN and its integer alter-ego,
C
     .     RWORK(NN+3*MAX+1), RWORK(NN+4*MAX+1), IWORK(NN+4*MAX+1), 
C
C NOBS and its real alter-ego, INDEX
C
     .     IWORK(NN+5*MAX+1), RWORK(NN+5*MAX+1), IWORK(NN+6*MAX+1) )
C
      CALL BYEBYE
      END!
C
...
C
      ...
      CHARACTER FILE(*)*30, OFILE*30, EXTEND*30, SWITCH*30
     ...
     .     NOC(MAXCON,MAXFRM)
      ...
      REAL RWHICH(MAX,*), ROBS(*)
      INTEGER IDMAST(MAX), LASTAR(0:MAXFRM), NOK(MAXFRM)
      INTEGER IWHICH(MAX,*), NOBS(*), INDEX(*), NLINE(*), IRMIN(*)
...
C
      CHARACTER LFILE*30, POSIT1*9, POSIT2*9, STRNG*9, ANSWER*1
c     DOUBLE PRECISION DTRMNT
...
D     REAL DEL, DELP1                                                !sm_
     ...
     .     PWT1, STEP, PWT1I, CCLAMP
      ...
      INTEGER NERROR, NFEW, NENUF, NFOLD
      LOGICAL TFR, SHRINK
D     LOGICAL PLOT, PAUSE                                            !sm_
      DATA DMAG/MAXFRM*99.999/, NOK/MAXFRM*0/, MIN/8/, CCLAMP/1./
D     DATA DEL /0.6/                                                 !sm_
C
C=======================================================================
C
...
C
      ...
      PWT(1) = PWT1
 2000 NITER = NITER+1
      IF (NITER .GT. MITER) CCLAMP = 2.*REAL(MITER)/REAL(MITER+NITER)
D     if (plot) then
D        call sm_device ('xterm')
D        call sm_graphics
D        call sm_expand (1.2)
D        call sm_erase
D        call sm_alpha
D        pause = .true.
D     else
...
C
      ...
      CALL RECTFY (MM, NMASTR, INDEX, IRMIN)
      IF (IFIRST .EQ. 2) CALL RECTFY (RWHICH, NMASTR, INDEX, IRMIN)
C
      DO I=MINY,MAXY
         LAST1(I)=0
      END DO
...
C
         ...
         DO I=1,IFRM-1
            CALL RECTFY (RWHICH(NMASTR+1,I), NEW-NMASTR, 
...
D        a = 1.e20
D        b = -1.e20
D        do imastr=1,nmastr
D           l = iwhich(imastr,ifrm)
D           if (l .gt. 0) then
D              if (nobs(imastr) .ge. minuse) then
D                 xx = x(l)
D                 yy = y(l)
D                 if (xx .lt. a) a = xx
D                 if (xx .gt. b) b = xx
...
D           go to 2850
D        end if
D        call sm_limits (-4., 4., a-.03*(b-a), b+.03*(b-a))
cD        call sm_ticksize (1000., 10000., 0., 0.)
D        call sm_box (3, 2, 3, 0)
D        call sm_xlabel (text(33:length(text)))
D        do j=-3,3,2
D           call sm_relocate (real(j), a)
D           call sm_draw (real(j), b)
D        end do
D        do j=-2,2,2
D           call sm_relocate (real(j), a-.03*(b-a))
D           call sm_draw (real(j), b+.03*(b-a))
D        end do
C
C small tick marks
C
...
D           do k=-3,3
D              xx = real(k)/4.
D              xx = real(j) + delp1*xx/(del+abs(xx))
D              call sm_relocate (xx, a-.03*(b-a))
D              call sm_draw (xx, a-.015*(b-a))
D              call sm_relocate (xx, b+.03*(b-a))
D              call sm_draw (xx, b+.015*(b-a))
D           end do
D        end do
D        iii = 0
D        do 2750 imastr=1,nmastr
D           if (nobs(imastr) .lt. minuse) go to 2750
D           if (mm(imastr) .gt. 90.) go to 2750
D           if (iwhich(imastr,ifrm) .gt. 0) iii = iii+1
D2750    continue
D        nfold = max0(1,iii/1500)
D     end if
C
...
C
      ...
      DO 2800 IMASTR=1,NMASTR
         ...
         WT = (NOBS(IMASTR)-MINUSE+1.) / 
     .        ((S(L)+1.E-3)*SM(IMASTR)**1.5)
c    .        ((S(L)+1.E-5)*SM(IMASTR)**1.5)
C
...
C
     ...
     .                 C(I,J)+WT*TERM(I)*TERM(J)
            ...
            END IF
         END DO
D        if (plot .and. (mod(iii,nfold) .eq. 0)) then
D           if(iwhich(imastr,1) .le. 0) then
D              call sm_ptype (41.5, 1)
D           else
D              call sm_ptype (41., 1)
D           end if
D           dx = dx/radius
D           dy = dy/radius
D           dx = delp1*dx/(del+abs(dx))
D           dy = delp1*dy/(del+abs(dy))
D           call sm_relocate (dx-3., x(l))
D           call sm_dot 
D           call sm_relocate (dx-1., y(l))
D           call sm_dot
D           call sm_relocate (dy+1., x(l))
D           call sm_dot
D           call sm_relocate (dy+3., y(l))
D           call sm_dot
D        end if
 2800 CONTINUE
D        if (plot) call sm_alpha
C
      iii = int(zz)
      IF (III .LT. MIN) THEN
         NTERM(IFRM) = 0
      ...
      ELSE IF ((III-MIN .EQ. 2) .AND. (MODE .EQ. 4)) THEN
         III = 4
      ELSE IF ((III-MIN .LE. 9) .AND. (MODE .GE. 6)) THEN
         III = 6
      ELSE IF ((III-MIN .LE. 15) .AND. (MODE .GE. 12)) THEN
         III = 12
      ELSE
         III = MIN0(MODE,20)
      END IF
...
C    .        / (1. + DABS(DCON(I) / CLAMP(I,IFRM)))
C        CON(I,IFRM) = CON(I,IFRM) + DCON(I) ! * CCLAMP
      END DO
      IF (III .EQ. 4) THEN
         CON(5,I) = -FLIP(I)*CON(4,I)
         CON(6,I) = FLIP(I)*CON(3,I)
...
D     if (plot) then
D        text = ' '
D        call ovrwrt (text(1:80), 2)
D        out(1) = drndff(dcon(1), 9, 3)
D        out(2) = drndff(dcon(2), 9, 3)
D        if (iii .ge. 3) then
D           do i=3,6
D              out(i) = drndff(dcon(i), 9, 5)
D           end do
D           if (iii .ge. 7) then
D              do i=7,iii
D                 out(i) = drndff(dcon(i), 9, 4)
D              end do
D           end if
D        end if
D        strng = rndoff(pwt(ifrm), 7, 3)
D        print 666,(out(i)(1:9),i=1,iii),strng(1:7)
D 666    format (12A / 24X, 9A)
C        print 666,(clamp(i,ifrm),i=1,iii)
C        print 666,(DCON(I) / (1. + DABS(DCON(I)/CLAMP(I,IFRM))),i=1,iii)
D        if (pause) then
D           call rdchar (5, text, i, istat)
D        else
D           call sleep (1)
D        end if
D        call sm_graphics
...
D        call sm_alpha
D        if (pause) then
D           if ((text(1:1).eq.'e').or.(text(1:1).eq.'E')) then
D              plot = .false.
D           else if ((text(1:1).eq.'r').or.(text(1:1).eq.'R')) then
D              pause = .false.
D           end if
D        end if
D     end if
...
C
      ...
      ELSE
C
C Maximum weight goes as the inverse of the relative scale.
C
         IF (NTERM(IFRM) .GE. MODE) THEN
            XX = 1./SQRT(ABS(CON(3,IFRM)*CON(6,IFRM)) + 
     .                   ABS(CON(4,IFRM)*CON(5,IFRM)))
            PWT(IFRM) = AMIN1( STEP*PWT(IFRM), sqrt(XX) )
         END IF
C
         I = 2*L
...
c      WRITE (6,603) POSIT1(1:J), STRNG
c  603 FORMAT (2X, A, ' stars within radius', A6)
      WRITE (6,603) POSIT1(1:J), radius
  603 FORMAT (2X, A, ' stars within radius', f9.2)
      CALL TBLANK
      ROLD = RADIUS
      CALL TBLANK
c
c ~~~~~~~~~~~~~~~~~~~~~~~~~
c
c	open (777, file='temporary.mch', status='unknown')
c         DO I=1,NFRM
c            IF (MODE .EQ. 4) THEN
c               CON(6,I)=FLIP(I)*CON(3,I)
c               CON(5,I)=-FLIP(I)*CON(4,I)
c            END IF
C
C
c            IF (MODE .LE. 6) THEN
c               WRITE (777,1109) FILE(I), (OUT(J), J=1,6), 
c     .              DMAG(I), SQRT(SMAG(I))
c            ELSE
c               DO J=7,MODE
c                  OUT(J) = DRNDFF(CON(J,I), 12, 9)
c               END DO
c               WRITE (777,1109) FILE(I), (con(J,i), J=1,6), 
c     .              DMAG(I), SQRT(SMAG(I)), (con(J,i), J=7,MODE)
c               WRITE (1,109) FILE(I), (OUT(J), J=1,6),                   ! definito sotto!
c     .              DMAG(I), SQRT(SMAG(I)), (OUT(J), J=7,MODE)
c            END IF
c         END DO
c         CLOSE (777)
c
...
C
         ...
         DO IFRM=1,NFRM
            CALL RECTFY (RWHICH(1,IFRM), NMASTR, INDEX, IRMIN)
         END DO
         DO IMASTR=1,NMASTR
            INDEX(IMASTR) = IMASTR
         END DO
...
c            WRITE (1,110) IDMAST(IMASTR), POSIT1, POSIT2,
c     .           MM(IMASTR), SQRT(SM(IMASTR)), A, NOBS(IMASTR),
c     .           SUMCHI, SUMSHP, STRNG, RR
c  110       FORMAT (I7, 2A9, F9.3, 2F9.4, I8, '.', 2F9.3, A9, F9.3)
            WRITE (1,110) IDMAST(IMASTR), xm(imastr), ym(imastr),
     .           MM(IMASTR), SQRT(SM(IMASTR)), A, NOBS(IMASTR),
     .           SUMCHI, SUMSHP, STRNG, RR
  110       FORMAT (I7, 2f10.3, F9.3, 2F9.4, I8, '.', 2f9.3, A9, F9.3)
         END DO
         CALL CLFILE (1)
      END IF
C
...
C
     ...
     .                 (9.9999,SQRT(AMAX1(0., S(J))))
            ...
            POSIT1 = RNDOFF(XM(IMASTR), 9, 3)
            POSIT2 = RNDOFF(YM(IMASTR), 9, 3)
c            WRITE (1,111) IDMAST(IMASTR), POSIT1, POSIT2,
c     .               ((DATUM(J,I), J=1,2), I=1,NFRM), SUMCHI, SUMSHP
c  111       FORMAT (I7, 2A9, 12F9.4:/ (24X, 12F9.4))
            WRITE (1,111) IDMAST(IMASTR), XM(IMASTR), YM(IMASTR),
     .               ((DATUM(J,I), J=1,2), I=1,NFRM), SUMCHI, SUMSHP
c  111       FORMAT (I7, 2f9.3, 12F9.4:/ (24X, 12F9.4))
c  111       FORMAT (I7, 2f10.3, 12F9.4:/ (24X, 12F9.4))
c  111       FORMAT (I7, 2f10.3, 12F10.4:/ (24X, 12F10.4))
  111       FORMAT (I7, 2f10.3, 12F11.4:/ (24X, 12F11.4))
         END DO
         CALL CLFILE (1)
      END IF
C
...
C
            IF (MODE .LE. 6) THEN
c               WRITE (1,109) FILE(I), (OUT(J), J=1,6), 
c     .              DMAG(I), SQRT(SMAG(I))
               WRITE (1,1109) FILE(I), (OUT(J), J=1,6), 
     .              DMAG(I), SQRT(SMAG(I))
c  109          FORMAT (1X, '''', A, '''', 2A10, 4A12, F9.3, 
c     .              F8.4, 14A12)
            ELSE
               DO J=7,MODE
                  OUT(J) = DRNDFF(CON(J,I), 12, 9)
               END DO
               WRITE (1,1109) FILE(I), (con(J,i), J=1,6), 
     .              DMAG(I), SQRT(SMAG(I)), (con(J,i), J=7,MODE)
 1109          FORMAT (1X, '''', A, '''', 2(f10.4,1x), 4(f12.9,1x), 
     .              f9.3, F8.4, 14(f13.8,1x) )
c 1109          FORMAT (1X, '''', A, '''', 2(f10.4,1x), 4(f12.9,1x), F9.3, 
c     .              F8.4, 14(f13.8,1x))
c               WRITE (1,109) FILE(I), (OUT(J), J=1,6), 
c     .              DMAG(I), SQRT(SMAG(I)), (OUT(J), J=7,MODE)
            END IF
...
C
 ...
 8102       CALL RDCHAR (1, TEXT, III, ISTAT)
            ...
            END DO
            READ (TEXT,117,END=8103) (LINE(J,I), J=1,III)
  117       FORMAT (132A1)
            IF (NL .EQ. 2) READ (1,*)
            GO TO 8101
C
...
C
 8105             LL = LL+1
                  ...
                  GO TO 8105
C
 8106             WRITE (TEXT,117) (CHAR(LINE(L,I)), L=1,LL-1)
   ...
   11             FORMAT (I7, 127A1)
     ...
     .                 SQRT(XX**2+YY**2)
  118             FORMAT (A, I7, 4F9.3)
                  K = K+1
               END IF
               IF ((K .NE. 0) .AND. (MOD(K,1000) .EQ. 0)) THEN
                  WRITE (TEXT,6) K, OFILE
                  CALL OVRWRT (TEXT(1:40), 2)
               END IF
            END DO
...
C##########################################################################
C
      SUBROUTINE  RDFRMS  (FILE, TFR, CHIMAX, SHPMAX, MAGMAX, SIGMAX, 
     .     NFRM, MAXMTR, WORK, MAXSTR, IDMAST, LASTAR, RCOL, RROW)
      IMPLICIT NONE
C
      REAL CHIMAX, MAGMAX, SHPMAX, SIGMAX, DM, RCOL(*), RROW(*)
...
C
         IF (TESTS) THEN
            ...
            IF (FILE(IFRM)(L-2:L) .EQ. 'nmg') ALS = .FALSE.
            IF (FILE(IFRM)(L-2:L) .EQ. 'mag') ALS = .FALSE.
...
c            READ (TEXT,*) ID, (WORK(ISTAR,J), J=1,4),
           READ (TEXT,*,ERR=1020) ID, (WORK(ISTAR,J), J=1,4),
     .        DM, DM, (WORK(ISTAR,J), J=5,6)
            IF (ID .LE. 0) GO TO 1020
            IF (ALS) THEN
               IF (WORK(ISTAR,5) .GT. CHIMAX) GO TO 1020
               IF (ABS(WORK(ISTAR,6)) .GT. SHPMAX) GO TO 1020
            END IF
         END IF
         IF (WORK(ISTAR,3) .GT. 90.) THEN
            GO TO 1020
         ELSE IF (ALS) THEN
            IF (WORK(ISTAR,3) .GT. MAGMAX) THEN
               GO TO 1020
            ELSE IF (WORK(ISTAR,4) .GT. SIGMAX) THEN
               GO TO 1020
            END IF
         END IF
         IMAX = IMAX+1
