...
c      PARAMETER (MAXDAT=  400 000 000, MAXMTR=3 000 000, MAXFRM=555)
c      PARAMETER (MAXWHI=  400 000 000)

c      PARAMETER (MAXDAT= 220 000 000, MAXMTR=1 000 000, MAXFRM=350)   !str
c      PARAMETER (MAXWHI=  400 000 000)
c
c      PARAMETER (MAXDAT= 100 000 000, MAXMTR=1 000 000, MAXFRM=600)    !danish95
c      PARAMETER (MAXWHI=  400 000 000)
c 
      PARAMETER (MAXDAT=   250 000 000, MAXMTR=   400 000, MAXFRM= 4950)    !danish98
      PARAMETER (MAXWHI=   120 000 000)
...
C
      ...
      PARAMETER (MAXFRM= 4950, MAXCON=20, MINY=-11 384, MAXY=15 000)
C
...
c	   if (file(i)(j:j) .eq. ':') then
c		if ((file(i)(j-1:j-1) .eq. 'p') .and.
c    .		  (file(i)(j+1:j+1) .eq. 'u')) pc(i) = .true.
c		if ((file(i)(j-1:j-1) .eq. 'w') .and.
c    .		  (file(i)(j+1:j+1) .eq. 'u')) wfc(i) = .true.
	      go to 82
	   end if
	end do
   82 CONTINUE
...
C        CON(I,IFRM) = CON(I,IFRM) + DCON(I) ! * CCLAMP
	...
	strng = rndoff(pwt(ifrm), 7, 3)
         print 666,(out(i)(1:9),i=1,iii),strng(1:7),'  ', file(ifrm)(1:length(file(ifrm)))
