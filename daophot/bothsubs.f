...
c
      ...
      include 'arrays.inc'
      character name*(*), commnt*72, switch*30
c
...
c
      ...
      if (imgtyp(imno) .eq. 'imh') then
         ...
         call ftdkey (imid(imno), keyword, ier)
      ier = 0
...
c
      ...
      include 'arrays.inc'
      character switch*30, image*30
      character coofile*30, magfile*30, psffile*30, profile*30, 
     .     grpfile*30
...
c Executable
c
c     print*,'opnpic ',pictur(imno),mode
      ier = 0
c
...
c
c     print*,'imgtyp ',imgtyp(imno), ichar(imgtyp(imno)(1:1)),
...
c
         ...
         go to 5000
      ELSE
c     print*,'fits didn''t work',ier
c     print*,message('fits',ier)
      end if
      if (imgtyp(imno) .ne. ' ') then
         imid(imno) = 0
         return
...
c     print*,'crepic imcrea',ier
         if (ier .ne. 0) then
            call stupid ('crepic: '//message('imh', ier))
...
c     print*,'crepic ftgiou',j,ier
         if (ier .ne. 0) then
            call stupid ('crepic: '//message('fits', ier))
...
c     print*,'before ftrsim',j,i,axlen
            call ftrsim (j, i, 2, axlen, ier)
            if (ier .ne. 0) then
               call stupid ('crepic: '//message('fits', ier))
c     print*,'crepic ftrsim',j,i,axlen,ier
               return
            end if
         end if
      end if
c
      if (ier .ne. 0) then
         call stupid ('crepic: '//message(imgtyp(imno), ier))
